...
# CONFIGURATION
# ----------------------------------------------------
DEFAULT_BOT_TOKEN = "8688074577:AAFzBSNJGiOKJClsCQDYLw0x9Uu_gx0K5Xk"
DEFAULT_OWNER_ID = 8806549778  # مالک اصلی

BOT_TOKEN = (DEFAULT_BOT_TOKEN or os.getenv("BOT_TOKEN", "")).strip()
try:
    OWNER_ID = int(str(DEFAULT_OWNER_ID) or os.getenv("OWNER_ID", "0"))
except ValueError:
    OWNER_ID = 0
...
# بارگذاری یا ساخت فایل ادمین‌ها
def load_admins():
    try:
...
if not OWNER_ID:
    logger.critical("OWNER_ID is not set or invalid.")
    sys.exit(1)
...
def get_main_keyboard(user_id=None):
    ...
    # دکمه مدیریت ادمین فقط برای مالک نمایش داده میشه
    if user_id and user_id == OWNER_ID:
        buttons.append([kbtn("👥 Manage Admins", "primary")])
    buttons.append([kbtn("📖 Tutorial", "success")])
...
# ----------------------------------------------------
# ADMIN MANAGEMENT COMMANDS & HANDLERS
# ----------------------------------------------------
@dp.message(Command("admins"))
@safe_handler
async def list_admins(message: types.Message):
    if message.from_user.id != OWNER_ID:
        await message.answer("⛔ Only the owner can manage admins.")
        return
    ...
    await message.answer(admins_text, reply_markup=get_main_keyboard(message.from_user.id))


@dp.message(F.text == "👥 Manage Admins")
@safe_handler
async def manage_admins_menu(message: types.Message):
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [kbtn("➕ Add Admin", "success"), kbtn("➖ Remove Admin", "danger")],
            [kbtn("📋 Admin List", "primary")],
            [kbtn("❌ Cancel", "danger")],
        ],
        resize_keyboard=True,
    )
    await message.answer("👥 **Admin Management:**", reply_markup=keyboard)


@dp.message(F.text == "📋 Admin List")
@safe_handler
async def show_admin_list_button(message: types.Message):
    if message.from_user.id != OWNER_ID:
        await message.answer("⛔ Only the owner can manage admins.")
        return
...
@dp.message(AdminManageState.waiting_for_admin_id)
@safe_handler
async def add_admin_execute(message: types.Message, state: FSMContext):
    if message.from_user.id != OWNER_ID:
        await state.clear()
        return
    ...
    if new_admin_id in ADMIN_IDS:
        await message.answer("⚠️ This user is already an admin.", reply_markup=get_main_keyboard(message.from_user.id))
    else:
        ...
        try:
            ...
            name = user.full_name or user.username or str(new_admin_id)
            await message.answer(f"✅ **{name}** (ID: `{new_admin_id}`) has been added as admin.", reply_markup=get_main_keyboard(message.from_user.id))
        except Exception:
            await message.answer(f"✅ User `{new_admin_id}` has been added as admin.", reply_markup=get_main_keyboard(message.from_user.id))
...
async def remove_admin_prompt(message: types.Message, state: FSMContext):
    ...
    admin_buttons = []
    for admin_id in ADMIN_IDS:
        if admin_id != OWNER_ID:  # مالک رو نمیشه حذف کرد
            try:
                user = await bot.get_chat(admin_id)
                name = user.full_name or user.username or str(admin_id)
                admin_buttons.append([kbtn(f"❌ {name} ({admin_id})", "danger")])
            except Exception:
                admin_buttons.append([kbtn(f"❌ {admin_id}", "danger")])
    
    if not admin_buttons:
        await message.answer("ℹ️ No other admins to remove.", reply_markup=get_main_keyboard(message.from_user.id))
        return
    
    admin_buttons.append([kbtn("❌ Cancel", "primary")])
    await state.set_state(AdminManageState.waiting_for_remove_admin_id)
    await message.answer(
...
@dp.message(AdminManageState.waiting_for_remove_admin_id)
@safe_handler
async def remove_admin_execute(message: types.Message, state: FSMContext):
    if message.from_user.id != OWNER_ID:
        await state.clear()
        return
    ...
    if message.text == "❌ Cancel":
        await state.clear()
        await message.answer("❌ Operation canceled.", reply_markup=get_main_keyboard(message.from_user.id))
        return
    
    # استخراج ID از متن دکمه
    try:
        import re
        ...
        await message.answer("❌ Invalid selection.", reply_markup=get_main_keyboard(message.from_user.id))
        await state.clear()
        return
    
    if admin_id == OWNER_ID:
        await message.answer("⛔ You cannot remove the owner.", reply_markup=get_main_keyboard(message.from_user.id))
    elif admin_id in ADMIN_IDS:
        ADMIN_IDS.remove(admin_id)
        save_admins(ADMIN_IDS)
        await message.answer(f"✅ Admin `{admin_id}` has been removed.", reply_markup=get_main_keyboard(message.from_user.id))
    else:
        await message.answer("❌ Admin not found.", reply_markup=get_main_keyboard(message.from_user.id))
...
async def handle_file(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"❌ File download failed: `{str(e)[:200]}`", reply_markup=get_main_keyboard(message.from_user.id))
        await state.clear()
        return
    ...
    if ext == ".zip":
        ...
        if project_type is None:
            shutil.rmtree(work_dir, ignore_errors=True)
            await message.answer("❌ Could not detect the project type inside the ZIP.", reply_markup=get_main_keyboard(message.from_user.id))
            await state.clear()
            return
        ...
        if not entry_path:
            shutil.rmtree(work_dir, ignore_errors=True)
            await message.answer("❌ No runnable file was found inside the ZIP.", reply_markup=get_main_keyboard(message.from_user.id))
            await state.clear()
            return
    else:
...
async def run_project_final(message: types.Message, state: FSMContext):
    ...
    if not runtime_ok:
        shutil.rmtree(work_dir, ignore_errors=True)
        await message.answer("❌ Could not run this project type in the current environment.", reply_markup=get_main_keyboard(message.from_user.id))
        await state.clear()
        return
    ...
    if not cmd:
        shutil.rmtree(work_dir, ignore_errors=True)
        await message.answer("❌ This project type is not supported.", reply_markup=get_main_keyboard(message.from_user.id))
        await state.clear()
        return
    ...
    try:
        ...
        shutil.rmtree(work_dir, ignore_errors=True)
        await message.answer(f"❌ Execution Error: `{str(e)[:300]}`", reply_markup=get_main_keyboard(message.from_user.id))
...
async def auto_stop_project(pid, delay, file_name, work_dir):
    ...
    if pid in RUNNING_PROCESSES:
        ...
        try:
            await bot.send_message(
                p_info["user_id"], f"⏰ **Time Expired!** Project `{file_name}` (PID: `{pid}`) was automatically stopped."
            )
        except Exception:
            pass
...
async def stop_project_action(message: types.Message, state: FSMContext):
    ...
    if pid in RUNNING_PROCESSES:
        ...
        del RUNNING_PROCESSES[pid]
        save_db()
        await message.answer(f"✅ PID `{pid}` stopped.", reply_markup=get_main_keyboard(message.from_user.id))
